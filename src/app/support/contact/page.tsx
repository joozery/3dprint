  ...
  const contacts = [
    {
      icon: Phone,
      label: "โทรศัพท์",
      value: settings.phone || "ยังไม่ได้ตั้งค่า",
      sub: settings.businessHours || "จันทร์ - เสาร์ 9:00 - 18:00 น.",
      color: "bg-blue-50 text-blue-600",
    },
    {
      icon: Mail,
      label: "อีเมล",
      value: settings.email || "ยังไม่ได้ตั้งค่า",
    ...
    },
    {
      icon: MessageCircle,
      label: "LINE OA",
      value: settings.lineId || "ยังไม่ได้ตั้งค่า",
      sub: "ตอบไวที่สุด แอดเพื่อนได้เลย",
      color: "bg-green-50 text-green-600",
    },
    {
      icon: MapPin,
      label: "ที่ตั้ง",
      value: settings.address || "ยังไม่ได้ตั้งค่า",
      sub: "ติดต่อล่วงหน้าก่อนเข้าพบ",
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-16">
            {contacts.map((c) => (
              <div key={c.label} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 text-center">
                ...
                <p className="font-bold text-slate-900 mb-1 break-all text-sm">{c.value}</p>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
              ...
              <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                <div>
                  ...
                  <textarea
                    rows={5}
                    placeholder="บอกเล่าความต้องการของคุณ..."
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-200 rounded-2xl h-56 flex items-center justify-center border border-slate-100">
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-400">{settings.address || "กรุงเทพมหานคร"}</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                ...
                <div className="space-y-2">
                  {settings.businessHours ? (
                    <div className="text-sm text-slate-700 font-medium py-2">
                      {settings.businessHours}
                    </div>
                  ) : (
                    <>
                      {[
                        ...
                        <div key={h.day} className="flex justify-between text-sm py-1.5 border-b border-slate-100 last:border-0">
                          ...
                          <span className={`font-semibold ${h.time === "ปิดให้บริการ" ? "text-red-500" : "text-slate-800"}`}>
                            {h.time}
                          </span>
                        </div>
                      ))}
